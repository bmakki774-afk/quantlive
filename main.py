"""
QuantLive Signal Platform — Entry Point
-----------------------------------------
...
def startup():
    log.info("=" * 60)
    log.info("  QuantLive Signal Platform — Starting Up")
    log.info(f"  Symbol:  {config.SYMBOL}")
    log.info(f"  Account: ${config.ACCOUNT_SIZE:,.0f}")
    log.info(f"  Schedule: every 15 min (:00/:15/:30/:45 UTC)")
    log.info("=" * 60)
    ...
    log.info("Running initial pipeline cycle on startup…")
    try:
...
def main():
    ...
    # Fire every 15 minutes: :00, :15, :30, :45
    scheduler.add_job(
        scheduled_pipeline,
    ...
    # Heartbeat every 30 minutes at :05 and :35
    scheduler.add_job(
        hourly_heartbeat,
    ...
    log.info("Scheduler started. Waiting for next :00 or :30…")
    try:
        scheduler.start()
