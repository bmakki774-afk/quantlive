...
Detects the building blocks of the ICT methodology:
  ...
  • Fair Value Gaps (FVGs) — bullish and bearish
  • Swing Highs / Swing Lows (BSL / SSL)
  • Equal Highs / Equal Lows (high-density stop clusters)
  • Round number levels
  • Order Blocks (simplified — last opposing candle before FVG)
  • Kill Zone detection (London / New York)
  • BSL/SSL sweep detection (price crosses level and returns)
"""
import logging
...
EQUAL_LEVEL_TOLERANCE = 0.5   # pts — two levels within this = "equal"
...
def detect_fvgs(df: pd.DataFrame, timeframe: str) -> list[FVG]:
    ...
    Bullish FVG: df.loc[i-2, 'high'] < df.loc[i, 'low']
      → gap between candle i-2 upper wick and candle i lower wick
...
def detect_equal_levels(
    ...
    for i, lv in enumerate(levels):
        ...
        # If clustered → upgrade to equal high/low with heavy density
        if len(cluster) > 1:
...
def detect_round_numbers(
    ...
    For gold: every $50 is a round number ($3,000, $3,050 …).
    """
    levels = []
            ...
            # Bearish displacement → look for last bullish candle before it
                ...
                if prev["close"] > prev["open"]:
                    obs.append(OrderBlock(
                        direction="bearish",
                        timeframe=timeframe,
                        zone_low=float(prev["close"]),  # body low of bullish OB
...
def check_mss(df: pd.DataFrame, direction: str, lookback: int = 5) -> bool:
    ...
    For a SELL setup (swept BSL above): we need a bearish close through the level.
    For a BUY setup (swept SSL below): we need a bullish close through the level.

    Simplified: checks if in last `lookback` candles there are 2+ consecutive
    closes in the opposite direction of the sweep, signalling MSS.
    """
    ...
    if direction == "bearish":
        # Looking for 2+ consecutive closes heading down
...
def map_stop_clusters(
    price: float,
    direction: str,  # 'buy' (stops below) or 'sell' (stops above)
    bsl_levels: list[LiquidityLevel],
    ssl_levels: list[LiquidityLevel],
    search_range: float = 200.0,
) -> list[LiquidityLevel]:
    """
    Step 0C — Collect all stop clusters in the stop-loss direction.

    For SELL setups: stops are ABOVE entry (BSL clusters).
    For BUY setups:  stops are BELOW entry (SSL clusters).
    """
    ...
    # Add round numbers in the search range
...
def calculate_hard_sl(
    ...
    entry_type: str,          # 'PHASE_B' or 'PHASE_D'
    trade_mode: str,          # 'INTRADAY' or 'SWING'
    buffer: float = config.BSL_SSL_BUFFER,
) -> tuple[float, float, str]:
    """
    Step 0C-4: Calculate hard SL from farthest stop cluster.
    Returns (hard_sl_price, distance_from_entry, sweep_probability).
    """
    # Determine minimum distance by entry type + mode
    min_map = {
...
def _is_fvg_mitigated(
    ...
    """
    FVG is only considered FULLY mitigated when a candle CLOSES through
    the 50% equilibrium (CE) of the gap — not just a wick into it.

    ICT rule: price can wick into an FVG and it remains valid.
    Only a candle CLOSE at or through the CE signals full mitigation.
    """
    future = df.iloc[fvg_idx + 1:]
    if future.empty:
        return False

    ce = (zone_low + zone_high) / 2   # 50% midpoint

    if direction == "bullish":
        # Mitigated: a candle closes at or below the CE (50% fill from above)
        return bool((future["close"] <= ce).any())
    else:
        # Mitigated: a candle closes at or above the CE (50% fill from below)
        return bool((future["close"] >= ce).any())


def _estimate_density(df: pd.DataFrame, price: float, level: str) -> str:
    """
    Rough density estimate based on how many candles touched near this level.
